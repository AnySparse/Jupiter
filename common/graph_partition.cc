...
#pragma omp parallel for
  for (size_t i = 0; i < nv; ++i)
  {
    ...
    new_ids[v] = i; // reindex
    if (v == begin_vids[subg_id])
      local_begin[subg_id] = i;
    if (v == end_vids[subg_id] - 1)
      local_end[subg_id] = i + 1;
  }
...
void PartitionedGraph::edgecut_partition1D()
{
  ...
  part_size = subgraph_size;
  std::vector<vidType> degrees(subgraph_size, 0);
  Timer t;
  for (int i = 0; i < num_subgraphs; ++i)
  {
    t.Start();
    ...
    local_begin[i] = begin_vids[i];
    local_end[i] = end_vids[i];
    auto v_size = end_vids[i] - begin_vids[i];
#pragma omp parallel for
    for (vidType v = begin_vids[i]; v < end_vids[i]; ++v)
    {
      vidType vid = v - begin_vids[i];
    ...
    parallel_prefix_sum<vidType, eidType>(degrees, offsets);
    auto ne = offsets[v_size];
    std::cout << "|V| = " << v_size << " |E| = " << ne << "\n";

    subgraphs[i] = new Graph();
    subgraphs[i]->allocateFrom(v_size, ne);
#pragma omp parallel for
    for (vidType v = begin_vids[i]; v < end_vids[i]; ++v)
    {
      vidType vid = v - begin_vids[i];
      ...
      subgraphs[i]->fixEndEdge(vid, end);
      vidType j = 0;
      for (auto u : g->N(v))
      {
        subgraphs[i]->constructEdge(begin + j, u);
        j++;
      }
    }
  }
  
  // local graph and remote graph split
  //   std::vector<vidType> local_degrees(v_size, 0);
  //   std::vector<vidType> remote_degrees(v_size, 0);
  //   Timer t;
  //   for (int i = 0; i < num_subgraphs; ++i)
  //   {
  //     t.Start();
  //     begin_vids[i] = v_size * i;
  //     end_vids[i] = begin_vids[i] + v_size;
  //     if (end_vids[i] > nv)
  //       end_vids[i] = nv;
  //     std::cout << "generating subgraph[" << i << "]: from vertex " << begin_vids[i] << " to " << end_vids[i] << "\n";
  ...
  //     for (vidType v = begin_vids[i]; v < end_vids[i]; ++v)
  //     {
  //       vidType local_vid = v - begin_vids[i];
  //       for (auto u : g->N(v))
  //       {
  //         if (begin_vids[i] <= u && u < end_vids[i])
  //         {
  //           local_degrees[local_vid]++;
  //         }
  //         else
  //         {
  //           remote_degrees[local_vid]++;
  //         }
  //       }
  //     }
  //     eidType *local_offsets = custom_alloc_global<eidType>(v_size + 1);
  //     eidType *remote_offsets = custom_alloc_global<eidType>(v_size + 1);
  //     parallel_prefix_sum<vidType, eidType>(local_degrees, local_offsets);
  //     parallel_prefix_sum<vidType, eidType>(remote_degrees, remote_offsets);
  //     auto local_ne = local_offsets[v_size];
  //     auto remote_ne = remote_offsets[v_size];
  //     std::cout << "|V| = " << v_size << " local |E| = " << local_ne << " remote |E| = " << remote_ne << "\n";

  //     local_subgraphs[i] = new Graph();
  //     remote_subgraphs[i] = new Graph();
  //     local_subgraphs[i]->allocateFrom(v_size, local_ne);
  //     remote_subgraphs[i]->allocateFrom(v_size, remote_ne);
  // #pragma omp parallel for
  //     for (vidType v = begin_vids[i]; v < end_vids[i]; ++v)
  //     {
  //       vidType vid = v - begin_vids[i];
  //       auto local_s = local_offsets[vid];
  //       auto local_e = local_offsets[vid + 1];
  //       auto remote_s = remote_offsets[vid];
  //       auto remote_e = remote_offsets[vid + 1];
  //       local_subgraphs[i]->fixEndEdge(vid, local_e);
  //       remote_subgraphs[i]->fixEndEdge(vid, remote_e);
  //       vidType local_j = 0, remote_j = 0;
  //       for (auto u : g->N(v))
  //       {
  //         if (begin_vids[i] <= u && u < end_vids[i])
  //         {
  //           local_subgraphs[i]->constructEdge(local_s + local_j, u);
  //           local_j++;
  //         }
  //         else
  //         {
  //           remote_subgraphs[i]->constructEdge(remote_s + remote_j, u);
  //           remote_j++;
  //         }
  //       }
  //     }
...
PartitionedGraph::PartitionedGraph(Graph *g, int nc, std::vector<int> cluster_ids) : num_vertex_chunks(nc), num_2D_partitions(nc * nc)
{
  auto nv = g->V();
...
void PartitionedGraph::fetch_partitions(std::string path, std::vector<int> clusters, Graph *&subg)
{
  ...
  for (auto cid : clusters)
    nv_subg += verts_of_clusters[cid].size();
  ...
  for (auto src_cid : clusters)
  {
    for (auto v : verts_of_clusters[src_cid])
    {                                          // v is the global vertex id
      ...
      for (size_t pid_index = 0; pid_index < clusters.size(); pid_index++)
      {
        ...
        for (int i = start; i < end; i++)
        {
          ...
          if (dst >= nv_subg)
            std::cout << "global_dst = " << global_dst << ", dst = " << dst << ", nv_subg = " << nv_subg << "\n";
          assert(dst < nv_subg);
