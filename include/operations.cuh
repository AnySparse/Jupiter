...
__forceinline__ __device__ T warp_reduce_max(T val) {
  T max_val = val;
  // max_val  = max(max_val, SHFL_DOWN(max_val, 16));
  // max_val += max(max_val, SHFL_DOWN(max_val, 8));
  // max_val += max(max_val, SHFL_DOWN(max_val, 4));
  // max_val += max(max_val, SHFL_DOWN(max_val, 2));
  // max_val += max(max_val, SHFL_DOWN(max_val, 1));
  // max_val  = max(max_val, SHFL(max_val, 0));
  max_val = max(max_val, SHFL_DOWN(max_val, 16));
  max_val = max(max_val, SHFL_DOWN(max_val, 8));
  max_val = max(max_val, SHFL_DOWN(max_val, 4));
  max_val = max(max_val, SHFL_DOWN(max_val, 2));
...
__forceinline__ __device__ unsigned count_larger(vidType bound, vidType *a, vidType size_a) {
  if (size_a == 0) return 0;
  unsigned thread_lane = threadIdx.x & (WARP_SIZE-1);            // thread index within the warp
  unsigned warp_lane   = threadIdx.x / WARP_SIZE;                // warp index within the CTA
  __shared__ unsigned count[WARPS_PER_BLOCK];
