...
template <typename T = vidType>
__forceinline__ __device__ T fuse_set_bs_cache(T *a, T size_a, T *b, T size_b, T *c, T *d, T *e, T *int_map)
{
  // if (size_a == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE - 1); // thread index within the warp
  int warp_lane = threadIdx.x / WARP_SIZE;         // warp index within the CTA
...
template <typename T = vidType>
__forceinline__ __device__ T fuse_set(T *a, T size_a, T *b, T size_b, T *c, T *d, T *e, T *int_map)
{
  if (size_a == 0 || size_b == 0)
    return 0;
...
template <typename T = vidType>
__forceinline__ __device__ T dif_int_set_bs(T *a, T size_a, T *b, T size_b, T upper_bound, T *c, T *d)
{
  int thread_lane = threadIdx.x & (WARP_SIZE - 1); // thread index within the warp
  int warp_lane = threadIdx.x / WARP_SIZE;         // warp index within the CTA
  __shared__ T dif_ab_count[WARPS_PER_BLOCK];
...
template <typename T = vidType>
__forceinline__ __device__ T dif_int_set(T *a, T size_a, T *b, T size_b, T upper_bound, T *c, T *d)
{
  if (size_a == 0 || size_b == 0)
    return 0;
...
template <typename T = vidType>
__forceinline__ __device__ T dif_int_set_bs(T *a, T size_a, T *b, T size_b, T *c, T *d)
{
  int thread_lane = threadIdx.x & (WARP_SIZE - 1); // thread index within the warp
  int warp_lane = threadIdx.x / WARP_SIZE;         // warp index within the CTA
  __shared__ T dif_ab_count[WARPS_PER_BLOCK];
...
template <typename T = vidType>
__forceinline__ __device__ T dif_int_set(T *a, T size_a, T *b, T size_b, T *c, T *d)
{
  if (size_a == 0 || size_b == 0)
    return 0;
...
template <typename T = vidType>
__forceinline__ __device__ T difference_set(T *a, T size_a, T *b, T size_b, T *c, bool flag)
{
  // if (size_a == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE - 1); // thread index within the warp
  int warp_lane = threadIdx.x / WARP_SIZE;         // warp index within the CTA
  __shared__ T count[1024 / 32];
...
template <typename T = vidType>
__forceinline__ __device__ T difference_num(T *a, T size_a, T *b, T size_b, bool flag)
{
  // if (size_a == 0) return 0;
  // assert(size_b != 0);
  int thread_lane = threadIdx.x & (WARP_SIZE - 1); // thread index within the warp
...
template <typename T = vidType>
__forceinline__ __device__ T difference_set(T *a, T size_a, T *b, T size_b, T *ancestor, int n, T *c)
{
  if (size_a == 0 || size_b == 0)
    return 0;
  // int thread_lane = threadIdx.x & (WARP_SIZE - 1); // thread index within the warp
  ...
  //   bool found = valid && !binary_search_2phase(search, cache, key, search_size);
  ...
  for (auto i = thread_lane; i < size_a; i += WARP_SIZE)
  {
    ...
    if (valid && !binary_search_2phase(b, cache, key, size_b))
      found = 1;
    unsigned mask = __ballot_sync(active, found);
    auto idx = __popc(mask << (WARP_SIZE - thread_lane - 1));
    if (found)
