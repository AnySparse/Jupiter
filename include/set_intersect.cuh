...
template <typename T = vidType>
__forceinline__ __device__ T intersect_num_bs(T* a, T size_a, T* b, T size_b, T* ancestor, int n) {
  if (size_a == 0 || size_b == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE-1); // thread index within the warp
  int warp_lane   = threadIdx.x / WARP_SIZE;     // warp index within the CTA
  AccType num = 0;
  ...
  for (auto i = thread_lane; i < lookup_size; i += WARP_SIZE) {
    ...
    if (valid && binary_search(search, key, search_size))
      num += 1;
  }
  return num;
}
...
template <typename T = vidType>
__forceinline__ __device__ T intersect_num_bs(T* a, T size_a, T* b, T size_b, T upper_bound, T ancestor) {
  if (size_a == 0 || size_b == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE-1); // thread index within the warp
  int warp_lane   = threadIdx.x / WARP_SIZE;     // warp index within the CTA
  AccType num = 0;
  ...
  for (auto i = thread_lane; i < lookup_size; i += WARP_SIZE) {
    ...
    int is_smaller = key < upper_bound ? 1 : 0;
    int found = 0;
    if (key != ancestor && is_smaller && binary_search(search, key, search_size))
      found = 1;
    num += found;
    unsigned mask = __ballot_sync(active, is_smaller);
    if (mask != FULL_MASK) break;
...
template <typename T = vidType>
__forceinline__ __device__ T intersect_num_lower(T* a, T size_a, T* b, T size_b, T lower_bound) {
  if (size_a == 0 || size_b == 0) return 0;
  int thread_lane = threadIdx.x & (WARP_SIZE-1); // thread index within the warp
  int warp_lane   = threadIdx.x / WARP_SIZE;     // warp index within the CTA
  T num = 0;
  ...
  for (auto i = thread_lane; i < lookup_size; i += WARP_SIZE) {
    ...
    int found = 0;
    if (is_larger && binary_search_2phase(search, cache, key, search_size))
      found = 1;
    num += found;
    unsigned active = __activemask();
